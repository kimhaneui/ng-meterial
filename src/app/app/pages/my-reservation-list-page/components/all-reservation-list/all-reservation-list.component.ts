...
@Component({
    ...
    styleUrls: ['./all-reservation-list.component.scss'],
})
export class AllReservationListComponent extends BaseChildComponent implements OnInit, OnDestroy {
    ...
    async callReservationtListApi(cate) {
        ...
        if (this.totalCount === 0) {
            //처음 로딩시에만 안보이게 처리하고 무한스크롤로 추가될때는 true상태로 진행됨
    ...
    async increase() {
        ...
        //  * IC04,   // 티켓/투어, 액티비티
        //  * IC05,   // 묶음할인
        if (this.totalCount !== 0 && this.totalCount < this.pageCount) {
            // totalCount != 0 (첫번째 api 호출)이 아니고
        ...
        const tmpCateResult = _.uniqWith(await this.result.result.list); // api에서 limit갯수로 받아온 리스트
        this.cateResult = _.uniqWith(this.cateResult.concat(tmpCateResult)); // cateResult에 tmpCateResult 가 concat 된 리스트
    ...
    goDetailPage(code, cate) {
        ...
        const path = this.pagePath + qsStr; //+ '/' + base64Str;
        const extras = {
            relativeTo: this.route,
        };
