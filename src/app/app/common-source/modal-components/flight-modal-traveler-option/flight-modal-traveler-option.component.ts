...
@Component({
    selector: 'app-flight-modal-traveler-option',
    templateUrl: './flight-modal-traveler-option.component.html',
    styleUrls: ['./flight-modal-traveler-option.component.scss']
})
export class FlightModalTravelerOptionComponent extends BaseChildComponent implements OnInit, OnDestroy {
    ...
    vmInit() {
        ...
        this.vm.cabinClassNm = this.selectedOption.cabinClassNm;
    ...
    onSeatDesc() {
        ...
        this.modalRef2 = this.bsModalService.show(FlightModalChildrenInformaionComponent);
    }
        ...
        switch (type) {
            case 'adult':
                if (event == 'up') {
                    ...
                    if (this.vm.adultCount > 0) {
                        if (this.vm.adultCount === this.vm.infantCount) {
                            alert('성인의 인원수보다 유아의 인원수를 더 많이 선택할 수 없습니다');
                            return;
                        }
                        this.vm.adultCount -= 1;
                    }
            ...
            case 'child':
                if (event == 'up') {
                    if (this.vm.infantCount < 2) {
                        if (this.vm.infantCount === this.vm.adultCount) {
                            alert('성인의 인원수보다 유아의 인원수를 더 많이 선택할 수 없습니다');
                            return;
                        }
